...
func (db *DB) DeleteUpstreamServer(id int64) error {
	...
	// Delete the log file for this server
	if err := logger.GetServerLogger().DeleteServerLog(id); err != nil {
		// Log the error but don't fail the database operation
		fmt.Printf("Warning: failed to delete log file for server %d: %v\n", id, err)
	}
...
func (db *DB) GetStatistics() (map[string]interface{}, error) {
	...
	err := db.conn.QueryRow("SELECT COUNT(*) FROM tokens WHERE is_active = true").Scan(&activeTokens)
	if err != nil {
